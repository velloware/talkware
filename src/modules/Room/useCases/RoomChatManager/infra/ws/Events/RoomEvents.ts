...
export class RoomEvents {
  ...
  async onJoinChat(
    clientConnectProps: clientConnectProps | any,
  ): Promise<string> {
    if (
      clientConnectProps ||
    ...
    const roomManager = await RoomManager.createRoomManager(this.socket.id, {
  ...
  async wealComeMessage() {
    this.socket
      .to(this.roomManager.RoomId)
      .emit('log', `Log: Join Client => ${this.roomManager.Client.name}`);
    this.socket.emit(
      'message',
      `*Welcome ${this.roomManager.Client.name} To '${this.roomManager.RoomId}'*`,
    );
  }
  ...
  async onMessage(data: string) {
    ...
    this.socket
      .to(this.roomManager.RoomId)
      .emit('message', `${this.roomManager.Client.name}: ${message.data}`);
  }
  ...
  async onDisconnect() {
    wsLogger(
      this.socket,
      `Disconnect ${this.roomManager.Client.name}`,
      'Disconnect',
    );
  }
  ...
  async onChangeRoom(RoomId: string) {
    ...
    wsLogger(
      this.socket,
      `Change chat and Join chat ${this.roomManager.Client.name}`,
      'Join',
    );
  ...
  onChangeName(name: string) {
    const swap = this.roomManager.Client.name;
    ...
    wsLogger(
      this.socket,
      `Change name ${this.roomManager.Client.name}`,
      'ChangeName',
    );
    this.socket
      .to(this.roomManager.RoomId)
      .emit(
        'log',
        `Log: Client change name => ${swap} to ${this.roomManager.Client.name}`,
      );
  }
}
